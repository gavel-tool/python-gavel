...
class TestLogicParser(unittest.TestCase):
    ...
    def assertObjectEqual(self, result, expected):
        ...
        if isinstance(result, LogicElement) or isinstance(result, AnnotatedFormula) or isinstance(result, Problem) or isinstance(result, Proof) or isinstance(result, ProofStep):
